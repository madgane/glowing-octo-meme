...
\resp \review{
	\begin{enumerate}
		...
		\item The under-estimator is tight when there is at least one user in each BS that cannot be served by the current transmission as discussed in Appendix A. It is not tight when the objective is zero even for a single BS. In this case, we can regularize the objective with the total transmitted power as
		\begin{equation}
		\|\tilde{\mbf{v}}\|_q + \varphi \, \|\mbf{m}\|^2
		\end{equation} 
		where \me{\mbf{m}} is the stacked vector of all transmit precoders. Note that the regularized objective makes the inequalities (16b) and (16c) tight at the optimal solution by making the power constraint in (16d) loose. It is discussed in the Appendix A provided on the supplementary document.
	\end{enumerate}
	 }
...
\resp \review{We thank the reviewer for the comment. Rigorous convergence proof for the proposed algorithm is provided in Appendix B on the supplementary document. The uniqueness is also justified implicitly by the linear approximation performed on the DC constraint (16b), which makes the transmit precoders to be susceptible to the phase rotation. The discussion on the uniqueness is provided in Appendix B-D.}
