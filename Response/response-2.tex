...
\resp \review{We understand the issue raised by the reviewer. Please note that the distributed algorithm is performed for the convex subproblem presented in (20) and (28). Since the problem is convex, the distributed approach convergence can be guaranteed under certain conditions. These are discussed in Appendix C on the convergence analysis for the distributed algorithms.}
...
\resp \review{
	\begin{itemize}
		...
		\item We have updated the manuscript to include these details. In addition, the number of backlogged packets in the system for Fig. 3 is \me{[9,16,14,16,9,13,11,12]} bits. These informations are updated in the revised manuscript on Section V-B third paragraph.
	\end{itemize}
  }
