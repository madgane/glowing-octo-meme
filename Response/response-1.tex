...
\resp \review{We thank the reviewer for the comment. 
	\begin{enumerate}
		...
		\item The equivalence between the Q-WSRM scheme and the \me{\ell_2} norm objective in the JSFRA formulation can be seen easily when the queue size increases. We have clarified this point in the revised manuscript. Please refer to the first paragraph in Section III-B.
	\end{enumerate}
		}
...
\resp \review{We have updated the manuscript to include the discussions on the convergence of the distributed algorithm in Appendix B. The distributed algorithm attains the same stationary point as of the centralized algorithm if we let the inner loop in Algorithm 2 to converge.}
