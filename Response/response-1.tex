...
\resp \review{The discussions on the convergence of the centralized algorithms is provided in Appendix B and for the distributed algorithms in Appendix C. The discussion on the convergence to a stationary point is presented in Appendix B-E on the supplementary document.}
...
\resp \review{
	\begin{enumerate}
		...
		\item Note that for \me{\ell_2} norm in the objective, the objective decreases monotonically, which can be seen by setting \me{J_{\max} = I_{\max} = 1} in the distributed algorithm presented in Algorithm 2. Following the monotonicity discussion in Appendix B-B, we can show the convergence of the KKT based algorithm to a stationary point [35],[36]. 
		\item We have provided additional details on the convergence of the KKT based approach in the Appendix C on the supplementary document.
	\end{enumerate}}
