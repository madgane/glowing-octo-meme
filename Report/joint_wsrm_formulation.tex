...
\end{IEEEeqnarray}
where \me{\mbf{Q}(i)} denotes the stacked user queues at the \me{\ith{i}} slot and the inequality is due to the upper bound
\begin{equation}
\sset{\max(Q-t,0) + \lambda)}^2 \leq Q^2 + t^2 + \lambda^2 + 2 Q (\lambda - t).
\end{equation}
Now the conditional Lyapunov drift for the instant \me{i} is given by
\begin{IEEEeqnarray}{rCl}
\Delta(\mbf{Q}(i)) &\triangleq& \mathbb{E} \set{\mathrm{L}\sset{\mbf{Q}(i+1)} - \mathrm{L}\sset{\mbf{Q}(i)} \vert \mbf{Q}(i)} \IEEEyessubnumber \\
...
\end{IEEEeqnarray}
where the expectation depends on the control decisions made for the transmission variables \me{t_k} and the second term on the r.h.s is due to i.i.d assumption on the arrivals. We can bound the first term on the r.h.s by a constant \me{B} for all possible control actions taken for a given channel condition \cite{neely2010stochastic}, the resulting problem can be given as
\begin{equation}
