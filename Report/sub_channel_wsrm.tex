...
\end{equation}
where \me{t_{l,k,j}} denotes the rate corresponding to the user \me{k} on the \me{\ith{j}} sub-channel and \me{\ith{l}} spatial stream. \review{Inspite of the improved covergence speed over the \ac{JSFRA} approach, the \ac{SRA} scheme is sensitive to the order in which the sub-channels are selected. Heuristically, we can adopt \ac{MU}-\ac{MIMO} scheduling scheme in \cite{sus2006zfbf} to find an efficient subset of users and the corresponding sum rate using zero-forcing precoders in each sub-channel, which can be used to determine the selection order. Since the scheduling is based on the infinite buffer model, it is used only in the sub-channel ordering. However, as the number of users in the system increases, the \ac{SRA} scheme will be insensitive to the sub-channel ordering due to the multi-user diversity.}
