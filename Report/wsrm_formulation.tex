...
Let us consider the \eqn{\ith{j}} \ac{SCA} iteration in the \eqn{\ith{i}} \ac{AO} step to find transmit precoders by solving \eqref{eqn-9}. For brevity, let us consider \eqn{\mbf{m}} as the collection of all transmit precoders as \eqn{\mbf{m} = \{\mvec{m}{l,k,n}\}_{\forall l,\forall k, \forall n}}. Similarly, we denote \eqn{\mbf{w}} and \eqn{\mbfa{\beta}} as \eqn{\{\mvec{w}{l,k,n}\}_{\forall l,\forall k, \forall n}} and \eqn{\{\mvec{\beta}{l,k,n}\}_{\forall l,\forall k, \forall n}} respectively. Let \eqn{\{\mbf{m}^{(i)}_{j}, \mbfa{\beta}^{(i)}_{j}\}} be a minimizer obtained in the \eqn{\ith{(j-1)}} \ac{SCA} step. Since \ac{SCA} is an iterative scheme, the next operating point \eqn{\tilde{\mbf{u}}_{l,k,n}} for the \eqn{\ith{(j+1)}} step is given as \eqn{\mbf{z}^{(i)}_{j} \triangleq \{\mbf{m}^{(i)}_{j}, \mbf{w}^{(i-1)}_{\lpointx}, \mbfa{\beta}^{(i)}_{j} \}}. Note that  \eqn{\mbf{w}^{(i-1)}_{\lpointx}} is obtained by solving either \eqref{opt-rx} or by using \eqref{eqn-10} in the \eqn{\ith{(i-1)}} \ac{AO} step.
\begin{algorithm}
	\SetAlgoLined
	\DontPrintSemicolon
	\BlankLine
...
Upon convergence of the objective sequence or for some fixed \eqn{K_{\max}}, receivers are then updated using fixed \eqn{\mbf{m}^{(i)}_{\lpointx}} found in previous \ac{SCA} step. Once the receivers are updated, transmit precoders are again evaluated using the newly found receivers by solving \eqref{eqn-9} or \eqref{eqn-10}. The above procedure is repeated until \eqn{i \to \infty} or for some fixed \eqn{I_{\max}} as outlined in Algorithm \ref{algo-1}.

A feasible initial point \me{\tilde{\mbf{u}}_{l,k,n}} is obtained by fixing \me{\tilde{\mbf{m}}_{l,k,n}} with the respective single-user precoders satisfying total power constraint in \eqref{eqn-9.1d} and \me{\tilde{\mbf{w}}_{l,k,n}}'s are updated by using the \ac{MMSE} receiver in \eqref{eqn-10}. Now, for fixed transmit and receive beamformers, \me{\tilde{\mbf{\beta}}_{l,k,n}}'s are evaluated using \eqref{eqn-9.1c}. 	
	
%	consider modified objective \eqref{mod_obj} with a quadratic term instead of \eqref{eqn-6.1}. Moreover, we also bound \eqn{\beta_{l,k,n}} as \eqn{\beta_{l,k,n} \leq \beta_{\max}} for some large \eqn{\beta_{\max} \gg P_{\max}} without affecting the solution to ensure compactness of the feasible set, since \eqn{\beta_{l,k,n}} need not be bounded when \eqn{\gamma_{l,k,n} = 0}, \textit{i.e.}, when \eqn{\ith{l}} stream is inactive. Using these modifications, convergence of the sequence of iterates is discussed in Appendix \ref{sec-3.5}.}
\begin{comment}

Let \eqn{i} be the \ac{AO} iteration count and let \eqn{j} denotes the \ac{SCA} index. To find the transmit precoders, \eqref{eqn-9} is solved recursively by updating the \eqn{\ith{j}} \ac{SCA} operating point \eqn{\tmbf{x}_{j}^{(i)} \triangleq \{ \tmbf{m}_{j}^{(i)},\tmbf{\beta}_{j}^{(i)} \}} in the \eqn{\ith{i}} \ac{AO} iteration for fixed receivers \eqn{\tmbf{w}} as \eqn{\tmbf{x}_{j}^{(i)} = \mbf{x}_{j}^{(i)}}, where \eqn{\mbf{x}_{j}^{(i)} \triangleq \{ \mbf{m}_{j}^{(i)},\mbf{\beta}_{j}^{(i)} \}} is the solution for the problem \eqref{eqn-9} in the \eqn{\ith{j-1}} \ac{SCA} step.\footnote{We can also update the operating point as \eqn{\tmbf{x}_{j}^{(i)} = \tmbf{x}_{j-1}^{(i)} + \rho_{j} (\mbf{x}_{j}^{(i)} - \tmbf{x}_{j-1}^{(i)})}, where \eqn{\rho_j \in (0,1]} is the step size as discussed in \cite{scutari_1,yang_yang}.} The iterative update is performed until \eqn{j \to \infty} or for \eqn{J_{\max}} number of iterations. Let \eqn{\tmbf{m}^{(i)}} be the transmit precoders obtained by \eqref{eqn-9} upon convergence in the \eqn{\ith{i}} \ac{AO} step with fixed receivers \eqn{\tmbf{w}^{(i-1)}}. Now, for the fixed transmit precoders, the receivers are determined either by solving \eqref{opt-rx} iteratively or by using \eqref{eqn-10}. The above procedure is iterated until \eqn{i \to \infty} or for \eqn{I_{\max}} number of iterations. Algorithm \ref{algo-1} outlines this procedure. 
