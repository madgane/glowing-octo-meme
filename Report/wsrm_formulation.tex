...
\end{subeqnarray}
where \me{\bar{f}(\mbf{x},\mbf{x}^{(i)})} is the approximate function of \me{f(\mbf{x})} around the point \me{\mbf{x}^{\ast(i)}}. Using the assumptions in \eqref{sca-req}, the global optimal point of the relaxed formulation, which is a convex problem, satisfies the \ac{KKT} conditions of the original nonconvex problem, using \eqref{sca-req}. It shows that each relaxed formulation converges to the optimal point.
