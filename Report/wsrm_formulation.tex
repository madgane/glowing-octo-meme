...
\end{IEEEeqnarray}}
Since the scaling present in the optimal receiver \eqref{opt-rx} has no impact on the received \acp{SINR}, the \ac{MMSE} receiver in \eqref{eqn-10} can also be used without compromising the performance or the convergence behavior.
\begin{algorithm}
	\SetAlgoLined
	\DontPrintSemicolon
	\BlankLine
	...
	\KwInit{\me{i=0} and transmit precoders \me{\tilde{\mbf{m}}_{l,k,n}} randomly satisfying the total power constraint \eqref{eqn-9.1d}}
...
The proposed subproblems in \eqref{eqn-9} and \eqref{eqn-9--1} are solved in an iterative manner by updating the operating point from the previous iteration. The iterative algorithm is referred as \acl{QM} \ac{JSFRA} scheme, and it is outlined in Algorithm \ref{algo-1}. \review{The initial feasible point \me{\tilde{\mbf{u}}_{l,k,n}} is obtained by initializing \me{\tilde{\mbf{m}}_{l,k,n}} with the single user beamformers satisfying the total power constraint \eqref{eqn-9.1d}, \me{\tilde{\mbf{w}}_{l,k,n}} with the corresponding \ac{MMSE} receivers \eqref{eqn-10} and \me{\tilde{\mbf{\beta}}_{l,k,n}} is updated using \eqref{eqn-9.1c} with the corresponding precoders and receivers.} For a fixed receiver \me{\mvec{w}{l,k,n}}, the \ac{SCA} iteration is carried out until convergence or for predefined number of iterations, say, \me{J_{\max}} to find the optimal transmit precoders \me{\mvec{m}{l,k,n}}. Next, the receivers are updated based on either \eqref{opt-rx} or \eqref{eqn-10} using the fixed transmit precoders \me{\mvec{m}{l,k,n}}. This procedure is carried out until convergence or for fixed number of iterations \me{I_{\max}} as outlined in Algorithm \ref{algo-1}. The convergence proof is discussed in Appendix \ref{sec-3.5}.
