...
In order to find a tractable solution for \eqref{eqn-6}, we note that \eqref{eqn-6.4} is the only convex constraint with the involved variables. Thus, we need to deal with \eqref{eqn-6.2} and \eqref{eqn-6.3}. To this end, we resort to the \ac{AO} technique by fixing the linear receivers to solve for the transmit beamformers. For fixed receivers \me{\{\mvec{w}{l,k,n}\}}, \textit{i.e.}, we fix all receivers and not only the one related to \eqn{l,k,n}, the problem now is to find optimal transmit precoders \me{\{\mvec{m}{l,k,n}\}} which is still challenging. By using fixed \me{\mvec{w}{l,k,n}}, \eqref{eqn-6.3} can be written as a \ac{SOC} constraint. Thus, the difficulty is due to the non-convexity of the \ac{DC} constraint in \eqref{eqn-6.2}. Let 
\begin{IEEEeqnarray}{l}
g({\mbf{u}}_{l,k,n}) \triangleq \frac{ | \mvec{w}{l,k,n}^\herm \mvec{H}{l,k,n} \mvec{m}{l,k,n}|^2}{\beta_{l,k,n}}
\end{IEEEeqnarray}
be the r.h.s of \eqref{eqn-6.2}, where \me{{\mbf{u}}_{l,k,n} \triangleq \{\mvec{m}{l,k,n}, \mvec{w}{l,k,n},\beta_{l,k,n}\}}. Note that the function \me{g({\mbf{u}}_{l,k,n})} is convex for a fixed \me{\mvec{w}{l,k,n}}, since it is in fact the ratio between a quadratic form of \me{\mvec{m}{l,k,n}} over an affine function of \me{\beta_{l,k,n}} \cite{boyd2004convex}. The nonconvex set defined by \eqref{eqn-6.2} can be decomposed as a series of convex subsets by linearizing the convex function \me{g({\mbf{u}}_{l,k,n})} with its first order Taylor approximation around a fixed operating point \me{\tilde{\mbf{u}}_{l,k,n}} \cite{scutari_1,lanckriet2009convergence}, also referred to as \ac{SCA} in \cite{marks1978technical}. By using the reduced convex subset for \eqref{eqn-6.2}, the problem in \eqref{eqn-6} is solved iteratively by updating the operating point in each iteration.
...
\end{IEEEeqnarray}
Note that the scaling present in the optimal receiver \eqref{opt-rx} has no impact on the received \acp{SINR}, and therefore the \ac{MMSE} receiver in \eqref{eqn-10} can also be used. However, the rate of convergence might be different with the two receivers.
...
Upon convergence of the transmit precoders, the receivers are updated using the fixed transmit precoders \eqn{\mbf{m}^{(i)}_{\ast}} obtained from the previous \ac{SCA} step. Once the receive beamformers are obtained, the transmit precoders are again updated by solving \eqref{eqn-9} recursively with the newly found receivers. The above procedure is repeated until \eqn{i \rightarrow \infty} or for \eqn{I_{\max}} number of iterations. The iterative approach is outlined in Algorithm \ref{algo-1}.
...
Even though the iterative approach in Algorithm \ref{algo-1} converges in all our numerical simulations, in order to provide an analytical convergence proof of the iterates to a stationary point, we consider a modified objective by augmenting a quadratic term as in \eqref{mod_obj} instead of \eqref{eqn-6.1}. The convergence proof for the centralized solutions is provided in Appendix \ref{sec-3.5}.}
\begin{comment}
