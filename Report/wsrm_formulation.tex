...
In summary, for fixed linear receiver \me{\mvec{w}{l,k,n}} and operating point \me{ [ \, \tilde{p}_{l,k,n},\tilde{q}_{l,k,n},\tilde{\beta}_{l,k,n} \, ]^T}, the relaxed convex subproblem to find transmit beamformers is given by
...
Now, the optimal linear receivers for fixed transmit precoders \me{\mbf{m}_{j,i,n} \, \forall i \in \mc{U}, \, \forall n \in \mc{C}} are obtained by minimizing \eqref{eqn-6} with respect to \me{\mbf{w}_{l,k,n}} as
...
The proposed subproblems in \eqref{eqn-9} and \eqref{eqn-9--1} are solved iteratively by updating the operating point from the previous iteration. The iterative algorithm is referred as \acl{QM} \ac{JSFRA} scheme, and it is outlined in Algorithm \ref{algo-1}. \review{The initial feasible point \me{\tilde{\mbf{u}}_{l,k,n}} is obtained by initializing \me{\tilde{\mbf{m}}_{l,k,n}} with the single user beamformers satisfying the total power constraint \eqref{eqn-9.1d}, \me{\tilde{\mbf{w}}_{l,k,n}} with the corresponding \ac{MMSE} receivers \eqref{eqn-10}, and \me{\tilde{\mbf{\beta}}_{l,k,n}} is updated using \eqref{eqn-9.1c} with the chosen transmit precoders and the receivers.} For a fixed receiver \me{\mvec{w}{l,k,n}}, the \ac{SCA} iteration is carried out until convergence or for predefined number of iterations, say, \me{J_{\max}} to find an optimal transmit precoders \me{\mvec{m}{l,k,n}}. Next, the receivers are updated based on either \eqref{opt-rx} or \eqref{eqn-10} using the fixed transmit precoders \me{\mvec{m}{l,k,n}}. The iterative procedure is performed until convergence or for fixed number of iterations \me{I_{\max}} as shown in Algorithm \ref{algo-1}. The convergence analysis is given in Appendix \ref{sec-3.5}.
\begin{algorithm}
	\SetAlgoLined
	\DontPrintSemicolon
	\BlankLine
