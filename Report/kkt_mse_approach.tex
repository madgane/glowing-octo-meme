...
The Algorithm \ref{algo-4} outlines a practical approach of implementing the precoder design in a distributed manner using \ac{OTA} signaling of transmit precoders and the receive beamformers for certain iterations before the actual transmission of data is performed. Unlike \ac{PD} or \ac{ADMM} approach, all variables are updated at once, \textit{i.e}, the \ac{SCA} point of \me{\epsilon^{(i-1)}}, \ac{AO} update of \me{\mvec{w}{l,k,n}} and the dual variable \me{\alpha}, it is difficult to prove the convergence of the algorithm to the centralized point. If the step size \me{\rho < 1}, the algorithm will converge by following the arguments made for controlling overallocation problem and the nonincreasing objective function, since the earlier iterates are in fact the operating point for the current iteration. Note that the objective is nonincreasing but the queue devation is not.
