...
\underset{\substack{t_{l,k,n},\mvec{M}{k,n}, \\ \sigma_{l,k,n},\mvec{W}{k,n}}}{\text{minimize}} &\hspace{0.5cm}& \sum_{b \in \mc{B}} \sum_{k \in \mc{U}_b} \, a_k \, \Big ( Q_k - \sum_{n = 1}^N \sum_{l = 1}^{L} t_{l,k,n} \Big )^q \IEEEyessubnumber \label{kkt-mse-1.1} \\
...
\alpha^{(i)}_{l,k,n} &=& \alpha^{(i-1)}_{l,k,n} + \rho \left ( \frac{\left [ \sigma_{l,k,n}^{(i)} \right ]^+}{\epsilon_{l,k,n}^{(i)}} - \alpha^{(i-1)}_{l,k,n} \right ) \IEEEyessubnumber \label{kkt-mse-4.1} \\
...
The receive beamformers from the users can be informed to the coordinating \acp{BS} by using the precoded uplink pilot signaling, where the precoders used for the uplink pilots are the receive beamformers \me{\sqrt{\alpha_{l,k,n}^{(i-1)}}\mvec{w}{l,k,n}^{\ast(i-1)}} evaluated at the receivers. Upon receiving the uplink precoded pilots by the \ac{BS}, the effective channel \me{\sqrt{\alpha_{l,k,n}^{(i-1)}} \mvec{w}{l,k,n}^{\ast \, (i-1)} \mvec{H}{b,k,n}^{\mathrm{T}}} can be used in the expression \eqref{kkt-mse-4.3} to update the transmit precoders at the respective \acp{BS} \cite{komulainen2013effective}, \me{\mbf{x}^{\ast}} represents the conjugate of \me{\mbf{x}}. The algorithmic representation of the \ac{KKT} based scheme is shown in Algorithm. \ref{algo-4}.
\begin{algorithm}
 \SetAlgoLined
 \DontPrintSemicolon
 \BlankLine
 ...
 \Repeat{until convergence or \me{i \geq I_{\max}}}{
	...
	update the \ac{MSE} \me{\epsilon_{l,k,n}^{(i)}} and the throughput \me{t_{l,k,n}^{(i)}} using \eqref{kkt-mse-4.4} and \eqref{kkt-mse-4.5}. \;
	solve for the dual variables \me{\alpha_{l,k,n}^{(i)}} and \me{\sigma_{l,k,n}^{(i)}} using \eqref{kkt-mse-4.1} and \eqref{kkt-mse-4.2}. \;
	$i = i + 1.$ \;
  }
  \me{j = j + 1}. \;
  evaluate the receive beamforming vector \me{\mvec{W}{k,n}^{(i)}} using \eqref{kkt-mse-4.6}. \;
  exchange the receive precoders \me{\mvec{W}{k,n}^{(i)}} and the dual variables \me{\alpha_{l,k,n}^{(i)}} across the coordinating \acp{BS} in \me{\mc{B}} using the uplink precoded pilots. \;
  }
...
The iterative method presented in Algorithm \ref{algo-4} converges to the stationary point if the dual variables \me{\alpha_{l,k,n}} are allowed to converge. The convergence of the dual variable is guaranteed, since the problem is convex once we fix the receive precoders \me{\mvec{w}{l,k,n}} and the operating \ac{MSE} point \me{\epsilon_{l,k,n}} \cite{boyd2011distributed}. Once the dual variables are converged or iterated to a certain accuracy, the receivers are updated using the \ac{MMSE} objective. In this algorithm, when \me{t_k > Q_k}, 
