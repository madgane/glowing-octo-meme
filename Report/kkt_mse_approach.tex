...
\begin{IEEEeqnarray}{ll} \label{kkt-mse-4}
	...
	\alpha^{(i)}_{l,k,n} =& \alpha^{(i-1)}_{l,k,n} + \rho \left ( \tfrac{\sigma_{l,k,n}^{(i)}}{\epsilon_{l,k,n}^{(i)}} - \alpha^{(i-1)}_{l,k,n} \right ) \IEEEyessubnumber \label{kkt-mse-4.1}
\end{IEEEeqnarray}}}
Since the dual variables \me{\alpha^{(i)}} and \me{\sigma^{(i)}} are interdependent in \eqref{kkt-mse-4}, one has to be fixed to optimize for the other. So, the variable  \me{\alpha^{(i)}} is fixed as in \eqref{kkt-mse-4.1} to obtain the other variables in \eqref{kkt-mse-4}. At each iteration, the dual variables \me{\alpha^{(i)}} are updated linearly from the earlier \me{\alpha^{(i-1)}} by a step size of \me{\rho \in [0,1]}. When the allocated rate \me{t_k^{(i-1)}} is greater than the number of queued packets \me{Q_k} for a user \me{k}, the corresponding dual variable \me{\sigma^{(i)}} will be negative thereby forcing \me{\alpha_k^{(i)} < \alpha_k^{(i-1)}} as in \eqref{kkt-mse-4.1}. Once the \me{\alpha_k^{(i)}} is reduced, the precoder weight in \eqref{kkt-mse-4.3} is lowered to make the rate \me{t_k^{(i)} < t_k^{(i-1)}}.

The \ac{KKT} expressions in \eqref{kkt-mse-4} are solved in an iterative manner by initializing the transmit and the receive precoders \me{\mvec{M}{k,n},\mvec{W}{k,n}} with the single user beamforming and the \ac{MMSE} vectors. The dual variable \me{\alpha}'s corresponding to precoder weights are initialized with ones to have equal weights. The transmit precoders in \eqref{kkt-mse-4.3} depends on the \ac{BS} specific dual variable \me{\delta_b}, which can be found by the bisection method satisfying the power constraint \eqref{kkt-mse-1.4}. Note that in \eqref{kkt-mse-4}, the fixed \ac{SCA} operating point is given by \me{\tilde{\epsilon}_{l,k,n} = \epsilon_{l,k,n}^{(i-1)}}. Following approaches can be performed to decouple the precoder design problem with the limited signaling between the \acp{BS} in \me{\mathcal{B}}.

As a first approach, in this method, for a fixed the receive beamformers, \me{\alpha_{l,k,n}} and the operating \ac{SCA} point \me{\tilde{\epsilon}_{l,k,n}}, each \ac{BS} evaluates the transmit precoder \me{\mvec{m}{l,k,n}^{(i)}} and exchange the total interference contribution by using the updated precoder to the neighboring \ac{BS} users. On receiving this information, each \ac{BS} will update \eqref{kkt-mse-4.4}, \eqref{kkt-mse-4.5}, \eqref{kkt-mse-4.2}, and \eqref{kkt-mse-4.1}. For the fixed receive beamformer, the dual variables \me{\alpha_{l,k,n}} and the interference contributions are exchanged until convergence by finding the updated transmit precoders. Once the dual variables are converged, the receivers are updated by performing the downlink transmission. This procedure is iterated until convergence for a fixed \me{I_{\max}} iterations. The convergence is similar to the \ac{JSFRA} scheme with \ac{MSE} reformulation in Section \ref{sec-3.3}.

The following approaches are similar to the decentralization methods proposed in \cite{komulainen2013effective} for the sum rate maximization problem. In the this approach, after receiving the transmitted signal with the recent precoders from all \acp{BS} in \me{\mathcal{B}}, the users evaluate the \ac{MMSE} receiver in \eqref{kkt-mse-4.6} and notify it to the \acp{BS} using the precoded uplink pilots. Upon receiving, each \ac{BS} estimates the \ac{MSE} using \me{\epsilon_{l,k,n}^{(i)} = 1 - \mvec{w}{l,k,n}^{(i)\herm} \mvec{H}{b_k,k,n} \mvec{m}{l,k,n}^{(i)}}. Using the updated \ac{MSE}, \me{t_{l,k,n}^{(i)},\sigma_{l,k,n}^{(i)}}, and \me{\alpha_{l,k,n}^{(i)}} are evaluated using \eqref{kkt-mse-4.5}, \eqref{kkt-mse-4.2} and \eqref{kkt-mse-4.1}. The updated dual variables \me{\alpha_{l,k,n}} are exchanged between the \acp{BS} to evaluate the transmit precoders \me{\mvec{m}{l,k,n}^{(i+1)}} for the next iteration. 

Finally, in this approach, upon receiving the signal from all \acp{BS}, the users update the receive beamformer \me{\mvec{w}{l,k,n}}, the \ac{MSE} \me{\epsilon_{l,k,n}}, and the dual variables \me{\lambda_{l,k,n}} and \me{\alpha_{l,k,n}}. The updated \me{\alpha_{l,k,n}} and \me{\mvec{w}{l,k,n}} are notified the \acp{BS} using precoded uplink pilots with \me{\mvec{\tilde{w}}{l,k,n}^{(i)} = \sqrt{\alpha_{l,k,n}^{(i)}}\mvec{w}{l,k,n}^{\ast(i)}} and \me{\mvec{\bar{w}}{l,k,n}^{(i)} = \alpha_{l,k,n}^{(i)}\mvec{w}{l,k,n}^{\ast(i)}} as the precoders. On receiving the precoded uplink pilots, the \acp{BS} will use the effective channel \me{\mvec{H}{b,k,n}^{\mathrm{T}} \mvec{\tilde{w}}{l,k,n}^{(i)}} and \me{\mvec{H}{b,k,n}^{\mathrm{T}} \mvec{w}{l,k,n}^{(i)}} in \eqref{kkt-mse-4.3} to update the transmit precoders, where \me{\mbf{x}^{\ast}} is the complex conjugate of \me{\mbf{x}}. Algorithm \ref{algo-4} outlines the distributed precoder design using the \ac{KKT} based \ac{MSE} reformulated \ac{JSFRA} problem.
\begin{algorithm}
 \SetAlgoLined
 \DontPrintSemicolon
 \BlankLine
 ...
 \Repeat{until convergence or \me{i \geq I_{\max}}}{
	update \me{\mvec{M}{k,n}^{(i)}} using \eqref{kkt-mse-4.3}, and perform downlink transmission \;
	find \me{\mvec{W}{k,n}^{(i)}} using \eqref{kkt-mse-4.6} at each user \;
	evaluate \me{\epsilon_{l,k,n}^{(i)}}, \me{t_{l,k,n}^{(i)}}, \me{\sigma_{l,k,n}^{(i)}} and \me{\alpha_{l,k,n}^{(i)}} using \eqref{kkt-mse-4.4} and \eqref{kkt-mse-4.5}, \eqref{kkt-mse-4.2} and \eqref{kkt-mse-4.1} at each user with the updated \me{\mvec{W}{k,n}^{(i)}} \;
	using precoded uplink pilots, \me{\mvec{W}{k,n}^{(i)}} and \me{\alpha_{l,k,n}^{(i)}} are notified to all \acp{BS} in \me{\mathcal{B}} \;
	\me{i = i + 1} \;
 }
 }
