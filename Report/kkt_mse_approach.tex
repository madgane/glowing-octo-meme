...
The \ac{KKT} expressions in \eqref{kkt-mse-4} are solved in an iterative manner by initializing the transmit and the receive beamformers \me{\mvec{M}{k,n},\mvec{W}{k,n}} with the single user beamforming and the \ac{MMSE} vectors. The dual variable \me{\alpha}'s are initialized with ones to have equal priorities to the corresponding precoders. Using this, the transmit and the receive beamformers are evaluated using the expressions in \eqref{kkt-mse-4}. The transmit precoder in \eqref{kkt-mse-4.3} depends on the \ac{BS} specific dual variable \me{\delta_b}, which can be found by bisection method satisfying the total power constraint \eqref{kkt-mse-1.4}. Note that in \eqref{kkt-mse-4}, the fixed \ac{SCA} operating point is given by \me{\tilde{\epsilon}_{l,k,n} = \epsilon_{l,k,n}^{(i-1)}}. The \ac{KKT} expressions can be solved in a distributed manner using primal decomposition technique with a similar convergence behavior as discussed in Section \ref{sec-4.1}. 

We extend the decentralization methods discussed in \cite{komulainen2013effective}, for the problem of minimizing the total number of backlogged packets. After receiving the updated transmit precoders from all \acp{BS} in \me{\mathcal{B}}, each user evaluates the \ac{MMSE} receiver in \eqref{kkt-mse-4.6} and notify them to the \acp{BS} via uplink precoded pilots. On receiving pilot signals, \acp{BS} update the \ac{MSE} as 
\begin{equation}
\epsilon_{l,k,n}^{(i)} = \Re{ \lbrace 1 - \mvec{w}{l,k,n}^{(i)\herm} \mvec{H}{b_k,k,n} \mvec{m}{l,k,n}^{(i)} \rbrace }.
\end{equation}
Using the current \ac{MSE}, \me{t_{l,k,n}^{(i)},\sigma_{l,k,n}^{(i)}}, and \me{\alpha_{l,k,n}^{(i)}} are evaluated using \eqref{kkt-mse-4.5}, \eqref{kkt-mse-4.2} and \eqref{kkt-mse-4.1}. The updated dual variables \me{\alpha_{l,k,n}} are exchanged between the \acp{BS} to evaluate the transmit precoders \me{\mvec{m}{l,k,n}^{(i+1)}} for the next iteration. The \ac{SCA} operating point is also updated with the current \ac{MSE} value.

In order to limit the back-haul exchanges between \acp{BS}, in this approach, users will perform all processing required and \acp{BS} will update the precoders based on the feedback information from the users. Upon receiving the transmit precoders from \acp{BS}, each user will update the receive beamformer \me{\mvec{w}{l,k,n}}, the \ac{MSE} \me{\epsilon_{l,k,n}}, and the dual variables \me{\lambda_{l,k,n}} and \me{\alpha_{l,k,n}}. The updated \me{\alpha_{l,k,n}} and \me{\mvec{w}{l,k,n}} are notified to the \acp{BS} using two separate precoded uplink pilot symbols with \me{\mvec{\tilde{w}}{l,k,n}^{(i)} = \sqrt{\alpha_{l,k,n}^{(i)}}\mvec{w}{l,k,n}^{\ast(i)}} and \me{\mvec{\bar{w}}{l,k,n}^{(i)} = \alpha_{l,k,n}^{(i)}\mvec{w}{l,k,n}^{\ast(i)}} as the precoders. On receiving the precoded uplink pilots, each \ac{BS} use the effective channel \me{\mvec{H}{b,k,n}^{\mathrm{T}} \mvec{\tilde{w}}{l,k,n}^{(i)}} and \me{\mvec{H}{b,k,n}^{\mathrm{T}} \mvec{w}{l,k,n}^{(i)}} in \eqref{kkt-mse-4.3} to update the transmit precoders, where \me{\mbf{x}^{\ast}} is the complex conjugate of \me{\mbf{x}}. Algorithm \ref{algo-4} outlines the distributed precoder design using the \ac{KKT} based \ac{MSE} reformulated \ac{JSFRA} problem. The proposed scheme in Algorithm \ref{algo-4} is of practical importance due to low signaling requirements. The convergence proof for the \ac{MSE}-\ac{KKT} schemes are difficult due to the joint update of \me{\mvec{w}{l,k,n}} and the \ac{SCA} update \me{\tilde{\epsilon}_{l,k,n}}. The convergence of the proposed algorithm is verified for different exponent values by using numerical simulations.
\begin{algorithm}
 \SetAlgoLined
 \DontPrintSemicolon
 \BlankLine
