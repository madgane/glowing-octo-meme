...
\label{eqn-4.2}
\end{equation}
where \me{\gamma_{l,k,n}} is given by \eqref{eq:SINR} and the optimization variables are the transmit precoders \me{\mvec{m}{l,k,n}} and the receive beamformers \me{\mvec{w}{l,k,n}}.
...
\text{subject to} & \quad&\sum_{n = 1}^N \sum_{k \in \mathcal{U}_b} \sum_{l=1}^L \text{tr} \, (\mvec{m}{l,k,n} \mvec{m}{l,k,n}^\herm) \leq P_{{\max}}, \fall b, \IEEEyessubnumber \eqspace \label{eqn-4.3}
\end{IEEEeqnarray}
where \me{\tilde{v}_k \triangleq a_k^{1/{q}}v_k} is the element of vector \me{\tilde{\mbf{v}}}, and \me{a_k} is the weighting factor which is incorporated to control user priority based on their respective \ac{QoS}. Let \me{\mbf{M}_{k,n} \triangleq [ \, \mvec{m}{1,k,n} \, \mvec{m}{2,k,n} \dotsc \mvec{m}{L,k,n} \,]} be the matrix formed by stacking the beamformers associated with user \me{k} for \me{n^\mathrm{th}} sub-channel transmission. Similarly, let \me{\mbf{W}_{k,n} \triangleq [ \, \mvec{w}{1,k,n} \, \mvec{w}{2,k,n} \dotsc \mvec{w}{L,k,n} \,]} denotes the matrix formed by stacking the receive beamformers respectively\footnote{It can be easily extended for user specific streams \me{L_{k,n}} instead of using the common \me{L} streams for all users}. In \eqref{eqn-4.3}, we consider a \ac{BS} specific sum power constraint for each \ac{BS} across all sub-channels.
