...
Using the above approximation for the rate constraint, the problem defined in \eqref{eqn-mse-1} is solved for optimal transmit precoders \me{\mvec{m}{l,k,n}}, \acp{MSE} \me{\epsilon_{l,k,n}}, and the user rates over each sub-channel \me{t_{l,k,n}} for fixed receive beamformers. The optimization subproblem to find the transmit precoders for fixed receive beamformers \me{\mvec{w}{l,k,n}} is given by
...
In all simulations, the objective of Algorithm \ref{algo-1} converges. \reviewF{However, to discuss formal convergence of relaxed problems \eqref{eqn-9}, \eqref{eqn-9--1} and \eqref{eqn-mse-2}, following modifications are considered. (i) Instead of \eqref{eqn-6.1} and \eqref{eqn-mse-2.1}, we consider a regularized objective with quadratic term as in \eqref{mod_obj} to ensure strong convexity in objective function. (ii) To guarantee the boundedness of feasible sets in \eqref{eqn-9} and \eqref{eqn-9--1}, we limit \eqn{\beta_{l,k,n}} as \eqn{\beta_{l,k,n} \leq P_{\max}} to make the convergence analysis easier without affecting the solution. It restricts \eqn{\beta_{l,k,n}} from taking \eqn{\infty} as  \eqn{\gamma_{l,k,n} \to 0} for inactive users, since \eqref{eqn-6.3} need not be active. Using these, convergence of Algorithm \ref{algo-1} is discussed in Appendix \ref{sec-3.5}.}
