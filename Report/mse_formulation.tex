...
\begin{algorithm}
	...
	\KwOut{\me{\mbf{m}} and \me{\mbf{w} \: \fall l \in \set{1,2,\dotsc,L}} and \me{i=0,j=0}}
	\KwInit{\me{\tilde{\mbf{m}}} by satisfying total power constraint \eqref{eqn-9.1d}}
	update \me{\tilde{\mbf{w}}, \tilde{\mbf{\beta}}} using \eqref{eqn-10} and \eqref{eqn-9.1c} with \me{\tilde{\mbf{m}}} \;
	\Repeat{Queue convergence or \me{i \geq I_{\max}}}{
		\Repeat{\ac{SCA} convergence or \me{j \geq J_{\max}}}{
			solve for transmit precoders \me{\mvec{m}{l,k,n}} using \eqref{eqn-9}\;
			update the operating point \eqn{\tilde{\mbf{u}}_{l,k,n}^{(j+1)}} using \eqref{operating_point_update}\;
			update $j = j + 1$\;
		}
		solve for \me{\mbf{w}} using \eqref{eqn-9--1} or \eqref{eqn-10} and update \eqn{\tilde{\mbf{u}}_{l,k,n}^{(0)}}\;
		update $i = i + 1$ and set \me{j = 0}\;
	}
...
\end{IEEEeqnarray}
The optimal transmit precoders for a fixed receiver are obtained by solving the subproblem \eqref{eqn-mse-2} iteratively by updating the fixed \ac{MSE} point \me{\tilde{\epsilon}_{l,k,n}} with \me{{\epsilon}_{l,k,n}} from the previous iteration until termination as outlined in Algorithm \ref{algo-1}. The convergence proof follows the discussions in Appendix \ref{sec-3.5}.
\begin{comment}
