...
Following the similar arguments as in  Section \ref{sec-3.2.1}, we can see that the \ac{SCA} subproblems converges to a stationary point of the original nonconvex problem at each iteration. The uniqueness of the transmit and the receive precoders are justified if the phase ambiguity from the stationary solution is eliminated. It can be seen from the \ac{MSE} expression \eqref{eqn-mse-1.3} that the phase rotations are fixed for both transmit and the receive precoders at the stationary point, which is evident from the following reformulation of \eqref{eqn-mse-1.3} as
\iftoggle{single_column}{
\begin{equation}
