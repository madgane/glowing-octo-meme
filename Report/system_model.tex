...
\label{eqn-2a}
\end{equation}
where \me{[x]^+ = \max{ \lbrace x,0  \rbrace}} and \me{t_k} denotes the transmission in bits for user \me{k}. For \ac{MIMO} \ac{OFDM} system,
\begin{equation}
t_k(i) = \sum_{n = 1}^N \, \sum_{l = 1}^L \, t_{l,k,n}(i),
\end{equation}
where \me{t_{l,k,n} = \log_2(1 + \gamma_{l,k,n})} corresponds to the maximum rate achieved by Gaussian signaling on \me{l^\mathrm{th}} spatial stream and over \me{n^\mathrm{th}} sub-channel for the \ac{SINR} of \me{\gamma_{l,k,n}}. Note that the units of \me{t_k} and \me{Q_k} are in bits defined per channel use.