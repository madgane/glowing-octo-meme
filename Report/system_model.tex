...
\end{equation}
where \me{\enoise = N_0 \, \trace (\mvec{w}{l,k,n} \mbf{w}_{l,k,n}^\herm)} denotes the equivalent noise variance. \review{To reduce the overhead involved in feeding back the user channels, we consider a \ac{TDD} system in which \acp{BS} can estimate the downlink channels from the uplink ones using the so-called channel reciprocity.}
