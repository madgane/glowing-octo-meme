...
Let \me{Q_k} be the number of backlogged packets destined for user \me{k} at a given scheduling instant. The queue dynamics of user \me{k} are modeled using the Poisson arrival process with the average number of packet arrivals of \me{A_k = \mathbf{E}_i\{\lambda_k\}} packets/bits, where \me{\lambda_k(i) \sim \mathrm{Pois}(A_k)} represents the instantaneous number of packets arriving for user \me{k} at the \me{\ith{i}} time instant.\footnote{The unit can either be packets or bits as long as the arrival and the transmission units are similar.} The total number of queued packets at the \me{\ith{(i+1)}} instant for user \me{k}, denoted as \me{Q_k(i+1)}, is given by
\begin{equation}
