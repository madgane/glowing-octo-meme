
\documentclass[11pt,draftcls,onecolumn,letterpaper]{./../Styles/IEEEtran}

\usepackage{amsmath,amssymb}
\usepackage{acronym,etoolbox}
\usepackage[ruled]{algorithm2e}
\usepackage[dvipdfmx]{graphicx}
\usepackage[subrefformat=parens,labelformat=parens,labelseparator=period,caption=false]{subfig}
\usepackage{subeqnarray,multirow,cite,array,setspace,fixltx2e,mathtools,lineno}

\graphicspath{{./../Figures/}{./../Figures/Linux/}}
\DeclareGraphicsExtensions{.eps}

\input{shortcuts}
\input{acronyms}

\newtoggle{enable_dist_fig}
\newtoggle{single_column}
\toggletrue{single_column}
\togglefalse{enable_dist_fig}

\iftoggle{single_column}{
	\doublespacing
}

\title{Traffic Aware Resource Allocation Schemes for Multi-Cell \acs{MIMO}-\acs{OFDM} Systems}
\author{
	\IEEEauthorblockN{Ganesh Venkatraman\IEEEauthorrefmark{1} \thanks{This work is supported by the Finnish Funding Agency for Technology and Innovation (Tekes), Nokia Solutions Networks, Xilinx Ireland, Academy of Finland. Part of this work has been published in ICASSP 2014 conference.} \IEEEmembership{Student Member,~IEEE}, Antti T\"{o}lli\IEEEauthorrefmark{1} \IEEEmembership{Member,~IEEE}, Le-Nam Tran\IEEEauthorrefmark{2} \thanks{\IEEEauthorrefmark{2}Dr. L.-N. Tran is associated with the Department of Electronic Engineering, Maynooth University, Co. Kildare, Irealand, e-mail:ltran@eeng.nuim.ie} \IEEEmembership{Member,~IEEE}, and Markku Juntti\IEEEauthorrefmark{1} \IEEEmembership{Senior Member,~IEEE}}\thanks{\IEEEauthorrefmark{1}The authors are with the Centre for Wireless Communications (CWC), Department of Communications Engineering (DCE), University of Oulu, Oulu, FI-90014, (e-mail: \{ganesh.venkatraman, antti.tolli, markku.juntti\}@ee.oulu.fi).}}

\begin{document}

\maketitle

\acused{MU-MIMO}

\begin{abstract}
\input{abstract}
\end{abstract}

\begin{IEEEkeywords}
convex approximations, \ac{MIMO}-\acs{IBC}, \ac{MIMO}-\ac{OFDM}, precoder design, \acs{SCA}, \ac{WSRM}.
\end{IEEEkeywords}

\acresetall

\section{Introduction} \label{sec-1}
\input{introduction}

\section{System Model and Problem Formulation} \label{sec-2-3.2}
\subsection{System Model} \label{sec-2}
\input{system_model}
\subsection{Problem Formulation} \label{sec-3.2}
\input{problem_formulation_intro}

\section{Proposed Queue Minimizing Precoder Designs} \label{sec-3}
\input{queue_minimizing_intro}

\subsection{Queue Weighted Sum Rate Maximization (\acs{Q-WSRM}) Formulation} \label{sec-3.1}
\input{joint_wsrm_formulation}

\subsection{\acs{JSFRA} Scheme via \ac{SCA} approach} \label{sec-3.2.1}
\input{problem_formulation}
\input{wsrm_formulation}

\subsection{\ac{JSFRA} Scheme via \ac{MSE} Reformulation} \label{sec-3.3}
\input{mse_formulation}

\subsection{Reduced Complexity Resource Allocation} \label{sec-3.4}
\input{sub_channel_wsrm}

\section{Distributed Solutions} \label{sec-4}

\input{decomposition_solutions}

\subsection{Decomposition based Approaches} \label{sec-4a}

\subsubsection{Primal Decomposition Approach} \label{sec-4.1}

\input{primal_approach}
%\input{primal_algorithm}

\subsubsection{\ac{ADMM} approach} \label{sec-4.2}

\input{dual_approach}
%\input{dual_algorithm}

\subsection{Decomposition via \ac{KKT} Conditions for \ac{MSE} Formulation} \label{sec-4.3}

\input{kkt_mse_approach}

\section{Simulation Results} \label{sec-5}
\input{simulation_intro}

\subsection{Centralized Solutions}
\input{simulation_results}

\subsection{Distributed Solutions}
\input{simulation_distributed}

%acresetall
\section{Conclusions} \label{sec-6}
\input{conclusions}

\appendices

%\section{\acl{PD} approach} \label{a-2}
%\input{primal_approach_appendix}

\section{\ac{KKT} conditions for \ac{MSE} approach} \label{a-1}
\input{kkt_appendix}

\bibliographystyle{./../Styles/IEEEtran}
\bibliography{./../Library/kirja_survey}


\end{document}
