...
\underset{\substack{\gamma_{l,k,n}, \mvec{W}{k,n},\mvec{M}{k,n} \\ \beta_{l,k,n}, \mbfa{\zeta}^{\set{b}(i)}}}{\text{minimize}} &\quad & \| \tilde{\mbf{v}}_{b} \|_q + \mbfa{\nu}^{{\set{b}(i-1)}T} \left ( \mbfa{\zeta}^{\set{b}(i)} - \mbfa{\zeta}^{(i-1)}(b) \right ) + \frac{\rho}{2} \Big \| \, \underbrace{\mbfa{\zeta}^{(i-1)}(b)}_{\text{consensus}} - \underbrace{\mbfa{\zeta}^{\set{b}(i)}}_{\text{local}} \, \Big \|^2_2 \IEEEyessubnumber \label{eqn-dual-3a} \\
