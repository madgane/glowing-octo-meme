...
\begin{IEEEeqnarray}{rCl} \label{eqn-dual-3}
	\underset{\substack{\gamma_{l,k,n}, \mvec{W}{k,n},\mvec{M}{k,n} \\ \beta_{l,k,n}, \mbfa{\zeta}^{\set{b}(i)}}}{\text{minimize}} && \| \tilde{\mbf{v}}_{b} \|_q + \mbfa{\nu}^{{\set{b}(i-1)}T} \left ( \mbfa{\zeta}^{\set{b}(i)} - \mbfa{\zeta}^{(i-1)}(b) \right ) \vspace{-0.25cm} \nonumber \\
	&& + \frac{\rho}{2} \Big \| \, \underbrace{\mbfa{\zeta}^{\set{b}(i)}}_{\text{local}} - \underbrace{\mbfa{\zeta}^{(i-1)}(b)}_{\text{consensus}} \, \Big \|^2_2 \IEEEyessubnumber \label{eqn-dual-3a} \vspace{-0.25cm} \\
	\text{subject to} &\quad& \nonumber \\
...
\label{zeta_update}
\end{equation}
The dual variable vector \me{\mbfa{\nu}^{\set{b_k}}}, which is the stacked dual variables of the interference equality constraint at the \ac{BS} \me{b_k}, are updated using the subgradient as
...
%, \forall b,b_k \in \mc{B}, \forall k \in \bar{\mc{U}}_{b}
\end{equation}
...
\begin{algorithm}
 ...
 \Repeat{convergence or \me{i \geq I_{\max}}}{
    ...
    update \me{\tilde{\mbfa{u}}_{l,k,n}} using \eqref{eqn-wsrm-expr} and \eqref{eqn-6.3} for the \ac{SCA} approach (or)\;
    update \me{\tilde{\epsilon}_{l,k,n}} using \eqref{eqn-mse-1.3} with equality for the \ac{MSE} approach \;
    $i = i + 1$ \;
  }
 }
