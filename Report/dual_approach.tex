...
\begin{algorithm}
	...
	\ForEach{\ac{BS} \me{b \in \mc{B}}}{
		\Repeat{convergence or \me{i \geq I_{\max}}}{
			...
			send precoded downlink pilot with \me{\mbf{m}_{l,k,n}}\;
...
\reviewF{Assuming that the following conditions are satisfied by the distributed schemes in every \ac{SCA} step, \textit{i.e.}, (i) by ensuring the uniqueness of the minimizer, and (ii) by choosing \eqn{J_{\max}} to be sufficiently large to ensure strict monotonicity of the objective, convergence of the distributed algorithms follow the discussions presented in Appendices \ref{sec-3.5} and \ref{sec-dist-conv}. Moreover, even if the receivers are updated together with the transmit precoders as in Algorithm \ref{algo-3}, strict monotonic decrease of the objective sequence is still ensured. The reason is that the \ac{MMSE} receiver is optimal for the fixed transmit precoders obtained after each \ac{SCA} update. However, in practice, to reduce the amount of information exchange among the coupling \acp{BS}, often distributed schemes are iterated for few steps, say, \eqn{J_{\max}}. If so, then the convergence of the sequence of iterates generated by the distributed algorithm is not guaranteed, since it is difficult to show the monotonicity of objective in each \ac{SCA} step. The reason is that in each primal or \ac{ADMM} update, the global objective need not be decreasing monotonically.}
