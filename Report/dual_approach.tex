...
The convergence analysis for the distributed algorithms is provided in Appendix \ref{sec-dist-conv}. \review{In practice, to reduce the amount of information exchange among the coupling \acp{BS}, often distributed schemes are iterated for few number of steps, say, \eqn{J_{\max}}. If so, the convergence of the iterates is not guaranteed, since it is difficult to show the monotonicity of objective in each \ac{SCA} step. It follows from the fact that in each primal or \ac{ADMM} update, the global objective need not be decreasing monotonically, despite reaching the centralized solution upon convergence. However, if the distributed algorithms are performed for adequate number of iterations to ensure strict monotonicity of the objective in each \ac{SCA} step, then the limit point of the sequence of iterates is a stationary point of \eqref{eqn-6}.}
