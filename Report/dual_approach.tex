...
Using the formulation presented in \cite{boyd2011distributed,admm_robust}, we can write the \ac{BS} \me{b} specific \ac{ADMM} subproblem for the \me{\ith{i}} iteration as
...
\end{IEEEeqnarray}}
where \me{\mbfa{\zeta}^{(i)}_b} denotes the interference vector updated from the earlier iteration and \me{\mbfa{\nu}^{(i)}_b} represents the dual vector corresponding to the equality constraint at the \me{\ith{i}} iteration as
...
\begin{algorithm}
	...
	\ForEach{\ac{BS} \me{b \in \mc{B}}}{
		\Repeat{convergence or \me{i \geq I_{\max}}}{
			...
			send precoded downlink pilot with \me{\mbf{m}}\;
			evaluate \me{\mvec{w}{l,k,n}} at each user and inform to all \acp{BS} in \eqn{\mc{B}} using uplink precoded pilots \cite{komulainen2013effective} \;
...
The convergence analysis for the distributed algorithms is provided in Appendix \ref{sec-dist-conv}. \review{In practice, to reduce the amount of information exchange among the coupling \acp{BS}, often distributed schemes are iterated for few steps, say, \eqn{J_{\max}}. If so, the convergence of the iterates is not guaranteed, since it is difficult to show the monotonicity of objective in each \ac{SCA} step. It follows from the fact that in each primal or \ac{ADMM} update, the global objective need not be decreasing monotonically, despite reaching the centralized solution upon convergence. However, if the distributed methods are performed for an adequate number of iterations to ensure strict monotonicity of the objective in each \ac{SCA} step, then the limit point of the sequence of iterates is a stationary point of \eqref{eqn-6}. Additionally, if the receiver is updated together with the transmit precoder in each \ac{SCA} step as in Algorithm \ref{algo-3}, then the convergence of the iterates is still guaranteed. It follows from the feasibility of the solution and the strict monotonicity of the objective after the combined update.}
