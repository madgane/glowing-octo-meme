...
\reviewF{Assuming that the following conditions are satisfied by the distributed schemes in every \ac{SCA} step, \textit{i.e.}, (i) by ensuring the uniqueness of the minimizer, and (ii) the distributed methods are performed for \eqn{j \to \infty}, convergence analysis follow the discussions in Appendices \ref{sec-3.5} and \ref{sec-dist-conv}. Moreover, the convergence discussions in Appendices \ref{sec-3.5} and \ref{sec-dist-conv} are valid even if the receivers are updated together with the transmit precoders as in Algorithm \ref{algo-3}, since strict monotonic decrease of the objective sequence can still be ensured. The reason is that the \ac{MMSE} receiver is optimal for the fixed transmit precoders obtained after each \ac{SCA} update. However, in practice, to reduce the amount of information exchange among the coupling \acp{BS}, often distributed schemes are iterated for limited number of iterations, say, for \eqn{J_{\max}} only. In such a case, convergence of the distributed algorithms cannot not be ensured, since it is difficult to show that the objective value is monotonically decreasing at every \ac{SCA} step. It follows from the fact that in each primal or \ac{ADMM} update, the global objective cannot be guaranteed to decrease monotonically.}
