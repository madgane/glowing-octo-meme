...
Assuming that the following conditions are satisfied in each \ac{SCA} step, \textit{i.e.}, (i) by ensuring uniqueness of the minimizer, and (ii) by choosing \eqn{J_{\max}} to be sufficiently large in order to ensure strict monotonicity of the objective, \reviewF{then the convergence follows the discussions in Appendices \ref{sec-3.5} and \ref{sec-dist-conv}}. Moreover, if the receivers are updated together with the transmit precoders as in Algorithm \ref{algo-3}, then strict monotonic decrease of the objective sequence is still ensured. The reason is that the \ac{MMSE} receiver is optimal for the fixed transmit precoders obtained after each \ac{SCA} update. However, to reduce the amount of information exchange among the coupling \acp{BS}, often distributed schemes are iterated for few steps, say, \eqn{J_{\max}}. If so, convergence of the iterates is not guaranteed, since it is difficult to show the monotonicity of objective in each \ac{SCA} step. The reason is that in each primal or \ac{ADMM} update, the global objective need not be decreasing monotonically.
