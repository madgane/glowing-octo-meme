...
To prove the convergence of the transmit and the receive beamformers, we use the uniqueness in the mapping between the transmit and receive beamformers with respect to the objective function. The uniqueness of the function mapping, \textit{i.e}, one-to-one correspondence between the feasible solution and the objective value, is achieved due to the linear relaxation performed on the \ac{DC} constraint in problem \eqref{eqn-9} and the \ac{MSE} expression in \eqref{eqn-mse-2}. The linearized \eqref{eqn-8} can be written as
...
\end{multline}
where \eqn{\mvec{\tilde{H}}{b_k,k,n} = \mvec{w}{l,k,n}^\herm \mvec{\tilde{H}}{b_k,k,n}} and for \ac{MSE} expression as
...
When the objective function is zero, the uniqueness of the transmit precoders using \eqref{uniqueness-1} and \eqref{uniqueness-2} are not valid due to the inactive constraints \eqref{eqn-6.2} and \eqref{eqn-mse-1.2}. To get the unique set of transmit precoders when the objective is zero even for a single \ac{BS}, we can regularize the objective with the transmit power as discussed in Appendix \ref{a-3} without affecting the optimal solution.
