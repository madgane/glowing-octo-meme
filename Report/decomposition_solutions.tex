...
To decouple the problem in \eqref{eqn-decent-1}, the \ac{BS} specific vector \me{\mbfa{\zeta}_b} in \eqref{if_vector}, which are relevant for the \ac{BS} \me{b}, can either be fixed or treated as a variable in each iteration in accordance to the decomposition method. To decouple the precoder design independently, the effective downlink channel \me{\mvec{w}{l,k,n}^\herm \mvec{H}{b,k,n}, \forall k \in {\mc{U}}} are assumed to be known at each \ac{BS} \me{b} through uplink precoded pilot signaling from all the users in the system. Similarly to update the \ac{MMSE} receivers at the user \me{k}, the equivalent uplink channels \me{\mvec{m}{l,k^{\prime},n}^\herm \mvec{H}{b,k,n}^\herm, \forall k^{\prime} \in {\mc{U}_b}, \forall b \in \mc{B}} are assumed to be known through user specific precoded downlink pilot signaling as detailed in \cite{komulainen2013effective}.
