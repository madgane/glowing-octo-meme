In this section, we discuss the distributed precoder designs for the solutions in Sections \ref{sec-3.2.1} and \ref{sec-3.3} for \eqref{eqn-6}. Note that the convex formulation in \eqref{eqn-9} or \eqref{eqn-mse-2} requires a centralized controller to perform the precoder design for all users belonging to the \acp{BS} in \eqn{\mc{B}}. In order to design the precoders independently at each \ac{BS} with a minimal information exchange via backhaul, iterative decentralization methods based on the \acl{PD} and the \ac{ADMM} are considered herein.

Let us consider the convex subproblem with fixed receive beamformers \me{\mvec{w}{l,k,n}} presented in \eqref{eqn-9} based on the \ac{SINR} relaxation for the nonconvex constraint in \eqref{eqn-6.2}. The following discussions are equally valid for the \ac{MSE} based solution outlined in \eqref{eqn-mse-2} as well. Since the objective of \eqref{eqn-9} can be decoupled across each \ac{BS}, the centralized problem can be equivalently written as
...
\end{IEEEeqnarray}
where \me{\tilde{\mbf{v}}_b} denotes the vector of weighted queue deviations corresponding to users \me{k \in \mc{U}_b}.
