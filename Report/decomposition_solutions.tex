...
Now, to decouple the problem in \eqref{eqn-decent-1}, the \ac{BS} specific vector \me{\mbfa{\zeta}_b} in \eqref{if_vector}, which include all interference terms relevant for the transmission of \ac{BS} \me{b}, can either be fixed or treated as a variable in each iteration in accordance to the decomposition method. Since the \ac{BS} specific precoders are solved independently, we assume local channel information and queues are available at each \ac{BS} \me{b_k} together with the cross channel knowledge \me{\mvec{H}{b_k,k,n}, \forall k\in \bar{\mc{U}}_{b_k}} through precoded uplink sounding using the receive beamformers \me{\mvec{w}{l,k,n}} \cite{komulainen2013effective}. 
