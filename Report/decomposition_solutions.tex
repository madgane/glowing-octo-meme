...
\end{IEEEeqnarray}}
where \me{\tilde{\mbf{v}}_b} denotes the vector of of weighted queue deviation corresponding to users \me{k \in \mc{U}_b}.

Following the approaches presented in \cite{pennanen2011decentralized,tolli2011decentralized}, the coupling constraint \eqref{eqn-9.1c} or \eqref{eqn-mse-1.3} can be expressed by grouping the interference contribution from each \ac{BS} in \me{\mathcal{B}} as
...
The decentralization is achieved by decomposing the original convex problem in \eqref{eqn-decent-1} by an iterative master-slave model using either primal or \acl{DD} method. The coupling variables are controlled by the master problem and the slave problems solve for the optimization variables associated with itself only. Now, to decouple the problem in \eqref{eqn-decent-1}, the coupling variable \me{\zeta_{l,k,n,b}} in \eqref{eqn-decent-3}, which bind all \acp{BS} in \me{\mc{B}}, is replaced by a \ac{BS} specific variable \me{\zeta_{l,k,n,b}^{\{b_k\}}}. Using the \ac{BS} specific variable \me{\zeta_{l,k,n,b}^{\{b_k\}}} in \eqref{eqn-decent-3}, the subproblem in \eqref{eqn-decent-1} decouples completely for each \ac{BS} to find the precoders only for the users associated with the \ac{BS} \me{b_k}. The slave subproblems with the \ac{BS} specific interference variables are controlled by a master subproblem, which determines the interference variable \me{\zeta_{l,k,n,b}^{\{b_k\}}} in accordance with the global objective.
